...
//l : m
//n : q
//m_NumClassN: m_NumClassQ
//h:b
///////////////////////////////////////////////////////////////////


int MUTDP::Sample_C( unsigned char *b, 
	...
	vector<vector<int> > &m, double alpha0)
{
    ...
    // ph | a
    double *log_ph = new double[K];
    double *log_ph_temp = new double[K];
...
int MUTDP::Sample_B( unsigned char *b,
	unsigned char *h, 
	vector<unsigned char>  &ak,
	int qk, vector<int> &lk, vector<int> &lak0, vector<int> &lak1,
	vector<int> &mk,
	int *h_count,
	int *u, int I)
{
	...
	for(tt = 0; tt< numBlockT; tt++){
	    ...
	    if ( m_A[cc][tt- m_nBlockStart]=='0'){ 
		log_pA_B0 = log(alpha_b  + mk[tt]);
		log_pA_B1 = log(beta_b  + qk - mk[tt]);
	    }
	    else{
		log_pA_B1 = log(alpha_b  + mk[tt]);
		log_pA_B0 = log(beta_b  + qk - mk[tt]);
	    }
	    ...
	    pB_AH = pB_H.push_back(exp(log_pB_H + log_pA_B0));

	    log_pB_H = lgamma(alpha_h + laj1[tt])  +  lgamma(beta_h + pj - 
	    laj1[tt])  - (pk - laj1[tt]) * logB1;
	    pB_AH = pB_H.push_back(exp(log_pB_H + log_pA_B1));


	    normalize(pB_AH);
	    bt = sample_discrete(pB_AH);
	    if ( bt < 0 && bt < Ba)  assert(0);
	    bk.push_back((unsigned char)bt);
	}
	...
	for(tt = 0; tt< numBlockT; tt++){
	    ...
	    pB_AH = pB_H.push_back(exp(log_pB_H0 + log_pA_B0));
	    pB_AH = pB_H.push_back(exp(log_pB_H1 + log_pA_B1));


	    normalize(pB_AH);
	    bt = sample_discrete(pB_AH);
	    if ( bt < 0 && bt < Ba)  asser(0);
	    bk.push_back((unsigned char)bt);
	}
